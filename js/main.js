(() => {
  const sprite = {
    url: "https://drive.google.com/thumbnail?id=1bH6J0MmpYGCYEMnYPe8YOmo-MI1dpu2p&sz=w4000",
    cols: 3,
    rows: 3
  };
  ...
  const spritePos = p => ({x:(p.spriteCell?.[0]||0)*50,y:(p.spriteCell?.[1]||0)*50});
  const spriteCover = p => {
    const pos=spritePos(p);
    return `<div style="position:relative;width:100%;height:100%;overflow:hidden;background:#f4f4f2"><div aria-hidden="true" style="position:absolute;inset:0;background-image:url('${sprite.url}');background-repeat:no-repeat;background-size:300% 300%;background-position:${pos.x}% ${pos.y}%"></div><span style="position:absolute;right:12px;bottom:12px;padding:6px 10px;border-radius:999px;background:rgba(16,42,43,.92);color:#fff;font-size:11px;font-weight:700;box-shadow:0 6px 20px rgba(0,0,0,.16)">مشروع مستقل</span></div>`;
  };
  ...
  function renderGallery(){
    const list=activeProject.gallery||[]; if(!list.length)return;
    ...
    const img=view.querySelector('img'); img.addEventListener('click',()=>img.classList.toggle('zoomed'));
    counter.textContent=`${galleryIndex+1} / ${list.length}`; controls.hidden=list.length<2;
  }

  function renderSpriteProject(p){
    const pos=spritePos(p);
    view.innerHTML=`<div role="img" aria-label="معاينة ${p.title}" style="width:100%;aspect-ratio:4/3;border-radius:16px;background-image:url('${sprite.url}');background-repeat:no-repeat;background-size:300% 300%;background-position:${pos.x}% ${pos.y}%;background-color:#f4f4f2"></div>`;
  }
  ...
  function openProject(id){
    ...
    if(p.spriteCell) renderSpriteProject(p);
    else if(p.gallery?.length) renderGallery();
